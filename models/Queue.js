...
const Queue = class Queue{
    static getAll(userId){
        ...
        // return true
        const tasks = db.collection('queue').findOne({userId: (userId)});
        return tasks;
    }
    ...
    static getOne(user, ind){
        ...
        return db.collection('queue').findOne({userId: userId}).then(userData => {
    ...
    static update(user, ind, task){
        const db = getDB();
        return db.collection('queue').findOne({userId: new ObjectId(user._id)}).then(userData => {
            const tasks = {...userData.tasks};
            const keys = Object.keys(tasks);
            tasks[task] = false;
    ...
    static delete(user, ind){
        ...
        // console.log(userId);
        return db.collection('queue').findOne({userId: (user._id)}).then(userData => {
            const tasks = {...userData.tasks};
            const keys = Object.keys(tasks);
    ...
    static move(user, ind){
        ...
        return db.collection('queue').findOne({userId: new ObjectId(user._id)}).then(userData => {
            ...
            const task = keys[ind];
            delete tasks[keys[ind]];
            db.collection('queue').updateOne({_id: new ObjectId(userData._id)}, {$set: {tasks: tasks}});
